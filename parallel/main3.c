...
/* right, bot, back */
void CheckEdge(size_t b[3], int *edge, const size_t GDIM, size_t *VOLUME, const size_t BRICKSIZE, int *src, size_t bno) {
	/* left top front */
	for(size_t i=0; i<3; i++) {
		if(src[i]<0) {
			edge[i*2] = (src[i])*-1;
...
size_t ReadFile(MPI_File f, size_t *VOLUME, uint8_t *data, int *src, const size_t GBSIZE, int *edge, size_t bno)
{
	size_t cur = 0;
...
size_t GetNewLOD(MPI_File fin, MPI_File fout, size_t old_bpd[3], size_t *VOLUME, size_t current_bpd[3], size_t current_no_bricks, const size_t BSIZE, const size_t GDIM, size_t lod, int rank, int size)
{
...
int main(int argc, char* argv[])
{
	...
	size_t *offset = malloc(sizeof(size_t)*3);
	offset[0] = myoffsets[0]; 
	offset[1] = myoffsets[1]; 
	offset[2] = myoffsets[2]; 
	size_t *orig_offset = malloc(sizeof(size_t)*3);
	orig_offset[0] = offset[0];
	orig_offset[1] = offset[1];
	orig_offset[2] = offset[2];
	...
	src[0] = (b[0]%bricks_per_dimension[0]*BRICKSIZE)-GHOSTCELLDIM;
	src[1] = (b[1]%bricks_per_dimension[1]*BRICKSIZE)-GHOSTCELLDIM;
	src[2] = (b[2]%bricks_per_dimension[2]*BRICKSIZE)-GHOSTCELLDIM;
	int *edge = malloc(sizeof(int) * 6);
